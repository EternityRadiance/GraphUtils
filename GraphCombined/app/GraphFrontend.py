...
class GraphSearchApp:
    def __init__(self, root, embedded=False):
        """
        Инициализация приложения поиска графов
        ...
        if embedded:
            self.root = root
            self.embedded = True
            # Для embedded режима используем переданный root как основной контейнер
            self.main_container = tk.Frame(root, bg=CONFIG.UI.colors.BACKGROUND)
            ...
            # Используем main_container вместо root для размещения виджетов
            ...
            self.setup_container = self.root
            self.use_status_bar = True
    ...
    def setup_logging(self):
        ...
        if not self.embedded:
            ...
            self.logger.info("=== Graph Search Tab Initialized ===")

    def init_console(self):
        """Инициализация консоли"""
        try:
            self.console = init_console()
            log_system("Графический интерфейс поиска графов инициализирован")
    ...
    def setup_ui(self):
        ...
        # Основной фрейм
        main_frame = ttk.Frame(self.setup_container, padding=CONFIG.UI.padding.LARGE)
        main_frame.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))

        # Настройка весов
        if not self.embedded:
            self.setup_container.columnconfigure(0, weight=1)
            self.setup_container.rowconfigure(0, weight=1)
        ...
        # Статус бар (только если не встроено и используем статус бар)
        if self.use_status_bar:
    ...
    def animate_process_gradient(self, target_color, final_text=None):
        ...
        def animate(step):
            if step < len(all_colors):
                ...
                self.animation_running = False
                if final_text and hasattr(self, 'status_var'):
    ...
    def search_graphs(self):
        ...
        if self.use_status_bar and hasattr(self, 'status_var'):
        ...
        if self.current_results:
            ...
            if self.selected_graphs:
                ...
                if len(selected_list) <= 5:
                    ...
                    log_info(f"Первые 5 выбранных графы: {', '.join(selected_list[:5])}")
...
# Функция для отдельного запуска (оставлена для совместимости)
def run_frontend():
    ...
    def on_closing():
        console = get_console()
        if console:
            console.log_system("Приложение поиска графов завершено")
        root.destroy()
